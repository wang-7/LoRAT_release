...
def cal_SNR_exp_2_12(predict, truth):
    if torch.is_tensor(predict):
        predict = predict.detach().cpu().numpy()
    if torch.is_tensor(truth):
        truth = truth.detach().cpu().numpy()
...
def save(out_dir, data, pred, cond, batch, index=0):
...
def save_wifi(out_dir, data, pred, cond, batch, index=0):
    scio.savemat(os.path.join(out_dir, f'{batch}-{index}.mat'),{'pred':pred.numpy()})
...
def main(args):
    ...
    device = torch.device(
        'cpu') if args.device == 'cpu' else torch.device('cuda:0')
    # Lazy load model.
    print(model_dir)
    if os.path.exists(f'{model_dir}/weights.pt'):
        ...
        checkpoint = torch.load(f'{model_dir}/global_model.pt')
    else:
    ...
    elif args.task_id in [2, 11]:
        model = tfdiff_mimo(AttrDict(params)).to(device)
    elif args.task_id==3:
        model = tfdiff_eeg(AttrDict(params)).to(device)
    elif args.task_id==4:
        model = tfdiff_coord2CSI(AttrDict(params)).to(device)
    elif args.task_id==5:
        model = tfdiff_coord2CSI(AttrDict(params)).to(device)
    elif args.task_id in [6, 7]:
        model = tfdiff_WiFi(AttrDict(params)).to(device)
    ...
    with torch.no_grad():
        ...
        for features in tqdm(dataset, desc=f'Epoch {cur_batch // len(dataset)}'):
            ...
            if args.task_id in [5]:
                pred = diffusion.sampling(model, cond, device)
                # save
                np.savez(f'{out_dir}/data_{cur_batch}.npz', data=data.detach().cpu().numpy(), pred=pred.detach().cpu().numpy(), cond=cond.detach().cpu().numpy())
                cur_batch += 1
            if args.task_id in [0, 1, 4]:
                ...
                for b, p_sample in enumerate(pred_samples):
                    ...
                    if args.if_log:
                        ...
                        # 关闭figure
                        plt.close(fig)
                        writer.add_image(f'b{b}cond:{cond_samples[b]}', image, dataformats='WHC')
                    ...
                    elif args.task_id in [4]:
                        save(out_dir, d_sample.cpu().detach(), p_sample.cpu().detach(), cond_samples[b].cpu().detach(), cur_batch,b)
                cur_batch += 1
                # print(cur_batch)
            if args.task_id in [2, 3, 6, 7, 9, 10, 11, 12]:
                pred = diffusion.sampling(model, cond, device)
                # pred = diffusion.robust_sampling(model, cond, device)
                # print(cond.shape)
                # pred = diffusion.fast_sampling(model, cond, device)
                ...
                elif args.task_id in [2, 10, 11, 12]:
                    # print(data.shape, pred.shape)
                    snr_b = list(cal_SNR_MIMO(pred,data))
                    np.savez(f'{out_dir}/data_{cur_batch}.npz', snr_b=snr_b, data=data.detach().cpu().numpy(), pred=pred.detach().cpu().numpy(), cond=cond.detach().cpu().numpy())
                    print(snr_b)
                    snr_list+=snr_b
                    
                    # save_mimo(out_dir, data.cpu().detach(), pred.cpu().detach(), cond.cpu().detach(), cur_batch)
                elif args.task_id in [6, 7, 9]:
                    snr_b = cal_SNR_exp_2_12(pred,data)
                    print(snr_b)
        ...
        if args.task_id in [2, 3, 6, 7, 9, 10, 12]:
            np.save(f'{out_dir}/all_snr_list.npy', snr_list)
            print(f'Average SNR: {np.mean(snr_list)}.')
            print(f'Median SNR: {np.median(snr_list)}.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--if_log', type=bool, default=False, help='if turn on log')
